...
import { MapPin, Phone, Receipt, Search } from "lucide-react";
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden relative">
            ...
            <CardHeader className="pb-2 pt-8 px-8">
              <CardDescription className="font-black text-[20px] uppercase tracking-[0.2em] text-primary">
                上週營收
              </CardDescription>
            ...
            <CardFooter className="px-8 pb-8">
              <Progress
                ...
                value={((lastWeekSum._sum.amount ?? 0) * 100) / 50000}
              />
            </CardFooter>
          </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden relative">
            ...
            <CardFooter className="px-8 pb-8">
              <Progress
                className="h-2 bg-muted [&>div]:bg-primary"
                value={((lastMonthSum._sum.amount ?? 0) * 100) / 200000}
              />
            </CardFooter>
          </Card>
        </div>
            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow className="hover:bg-transparent border-border">
                  <TableHead className="py-6 px-8 font-black uppercase text-[12px] tracking-[0.2em] text-muted-foreground">
                    收件資訊
                  </TableHead>
                  <TableHead className="hidden xl:table-cell py-6 font-black uppercase text-[12px] tracking-[0.2em] text-muted-foreground">
                    配送細節
                  </TableHead>
                  <TableHead className="hidden md:table-cell py-6 font-black uppercase text-[12px] tracking-[0.2em] text-muted-foreground">
                    發票 / 編號
                  </TableHead>
                  <TableHead className="py-6 font-black uppercase text-[12px] tracking-[0.2em] text-muted-foreground">
                    狀態與操作
                  </TableHead>
                  <TableHead className="text-right py-6 px-8 font-black uppercase text-[12px] tracking-[0.2em] text-muted-foreground">
                    金額
                  </TableHead>
                </TableRow>
              </TableHeader>
                      ...
                      <TableCell className="py-8 px-8">
                        ...
                        <div className="flex items-center gap-1 mt-2 text-primary">
                          ...
                          <span className="text-sm font-black">
                            {order.shippingAddress?.phoneNumber || "無電話"}
                          </span>
                        </div>
                      </TableCell>
                        ...
                        <span className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest">
                          {order.createdAt
                            ? (() => {
                                ...
                                return isToday
                                  ? `今天 ${date.toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit", hour12: false })}`
