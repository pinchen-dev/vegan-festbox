...
export const createCheckoutSession = async ({
  ...
  if (boxSet && boxSet in PRODUCT_PRICES.boxSet) {
    price +=
      PRODUCT_PRICES.boxSet[boxSet as keyof typeof PRODUCT_PRICES.boxSet];
  }
  if (finish && finish in PRODUCT_PRICES.finish) {
    price +=
      PRODUCT_PRICES.finish[finish as keyof typeof PRODUCT_PRICES.finish];
  }
  ...
  if (Array.isArray(decoration)) {
    decoration.forEach((d) => {
      ...
      if (priceKey in PRODUCT_PRICES.decoration) {
        price +=
          PRODUCT_PRICES.decoration[
            priceKey as keyof typeof PRODUCT_PRICES.decoration
          ];
      }
    });
  }

  const { type, value, companyTitle } = invoiceInfo;

  let dbInvoiceValue: string | null = null;
  let dbCompanyTitle: string | null = null;

  if (type === "ELECTRONIC") {
    dbInvoiceValue = value || null;
    dbCompanyTitle = null;
  } else if (type === "COMPANY") {
    dbInvoiceValue = value || null;
    dbCompanyTitle = companyTitle || null;
  } else if (type === "PAPER") {
    dbInvoiceValue = null;
    dbCompanyTitle = null;
  }
  ...
  const order = await db.order.create({
    data: {
      ...
      invoiceType: type,
      invoiceValue: dbInvoiceValue,
      companyTitle: dbCompanyTitle,
