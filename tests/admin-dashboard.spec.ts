...
test("Admin Integration - End-to-End with Email Verification", async ({
  ...
  await page.waitForURL(/.*design.*/, { timeout: 20000 });
  await page.getByText("Step 3", { exact: true }).waitFor({ state: "visible" });
  ...
  await page.waitForURL((url) => url.href.includes("kinde.com"), {
    waitUntil: "commit",
  });
  ...
  const district = page
    .getByRole("combobox")
    .filter({ hasText: "區域" })
    .or(page.locator("button").filter({ hasText: "區域" }));
  ...
  const targetRow = page
    .locator("tr")
    .filter({ hasText: "管理員測試" })
    .first();
  await targetRow.scrollIntoViewIfNeeded();
  await expect(targetRow).toBeVisible({ timeout: 10000 });

  await targetRow.getByText("訂單詳情").click();
  ...
  // 驗證狀態已更新
  await expect(targetRow.getByRole("button", { name: "已出貨" })).toBeVisible({
    timeout: 10000,
  });
